...
public sealed class Phase6PerformanceTests
{
    ...
    public void ThousandItemProjectionAndDiffReportTimingWithoutTreatingTheInitialGoalAsAcceptanceLimit()
    {
        ...
        var notificationPresentation = service.BuildSnapshot(
            source,
            display,
            [],
            TimeZoneInfo.Utc);
        var notificationPlan = new EventNotificationPlanner().Plan(
            notificationPresentation.Events.Select(value => new EventNotificationSource(
                value.StableId,
                value.LocalStart,
                value.IsAllDay,
                value.IsMultiDay,
        ...
        _output.WriteLine(
            "Phase 6 pure-logic timing (excludes WPF layout/render): BuildSnapshot={0:F1} ms, initial diff={1:F1} ms, update diff={2:F1} ms, notification projection={3:F1} ms",
            buildTimer.Elapsed.TotalMilliseconds,
            applyTimer.Elapsed.TotalMilliseconds,
            updateTimer.Elapsed.TotalMilliseconds,
            notificationTimer.Elapsed.TotalMilliseconds);
        Assert.Equal(1_000, initial.Events.Count);
        Assert.Equal(1_000, notificationPresentation.Events.Count);
        Assert.NotNull(notificationPlan.Notification);
        ...
        Assert.True(
            notificationTimer.ElapsedMilliseconds < RegressionLimitMilliseconds,
            $"Notification projection exceeded the gross-regression limit: {notificationTimer.ElapsedMilliseconds} ms");
    }
