...
- **通知済みの記録は保存しない。ただし再起動で再通知されることはない**（2026-09-09 の実機受入 PP-47 で確認）。**起動直後の抑制（NTF-006）が、その時点で通知時刻を過ぎている予定を既通知として登録するためである。** 起動時点でまだ通知時刻に達していない予定は、通常判定で1回だけ通知される。
  - **当初この節は「再起動後、まだ開始前の予定は再度通知されることがある」と書いていたが、誤りだった。** NTF-006 と D140 の実装（`EventNotificationHostedService` は候補を1件以上観測した評価まで抑制を維持する）を反映していなかった。
