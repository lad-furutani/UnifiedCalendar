...
~~**未確認の事項。** 切り替え前に取得済みの refresh token が失効するかは実測していない。**7日失効が発行時の状態で決まるなら、切り替え後に一度だけ再認証が必要になる。** 切り替え後8日以上経っても再認証を求められないことをもって、目的の達成とする。~~ → **2026-09-18 に実測で達成を確認した（下記）。**

**この事項は実機では当てはまらなくなった（2026-09-09）。** 1.1.0 の受入後に v2 設定を 1.0.0 が隔離したため、**同日 17:15 に3アカウントを再認証した。現在のトークンはすべて切り替え後（本番環境）に発行されたものである。** したがって残る判定は「本番環境で発行した refresh token が7日で失効しないこと」だけとなり（起算日は 2026-09-09 17:15）、**下記のとおり 2026-09-18 に達成を確認した。**

### 7日失効が解消したことの実測（2026-09-18）

**達成を確認した。** 起算 2026-09-09 17:15 から **8日16時間43分**が経過し、**再認証を求められなかった。** 8.6 の基準「切り替え後8日以上」を満たす。

- **判定の方法。** 2026-09-17 の夜にタスクトレイの「終了」でアプリを完全終了し、翌 2026-09-18 09:58 に起動した。**保存済みアクセストークンの寿命（約1時間）を大きく超えて停止していたため、起動後の同期は refresh token の使用を伴う。** キャッシュ表示（FUN-006）で通常どおり見えることは判定の根拠にならないため、この経路を通した。
- **結果。** 同意画面は出ず、設定画面のアカウント3件（Google 2件、Microsoft 1件）がすべて**「有効・正常・最終正常更新: 今日 09:58」**。メイン画面にアカウント警告（⚠）は出ていない。
- **Google 2件のうち1件は登録所有者（`lsi.furutani@gmail.com`）以外のアカウントである。** 未検証アプリの一般利用者でも7日で失効しないことを併せて確認した。
- **結論。** 「テスト」の間に生じていた7日ごとの再認証は、**「本番環境（未検証）」への切り替えで解消した。** 残る制約（警告画面、同意済みユーザー数の上限、Google が未検証アプリを将来制限しうること）は上記のとおり変わらない。
