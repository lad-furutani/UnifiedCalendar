...
- ~~**Google の公開ステータス切り替え（2026-09-08）で7日ごとの再認証が解けたことの実測。**~~ → **完了（2026-09-18）。達成を確認した。** 起算 2026-09-09 17:15 から **8日16時間43分**が経過し、再認証を求められなかった。前夜にアプリを完全終了し、翌朝 09:58 の起動で refresh token を使う経路を通したうえで、3アカウントとも「正常・最終正常更新: 今日 09:58」であることを確認した。**8.6 の記述は見込みから実測へ書き換えた**（8.6「7日失効が解消したことの実測」）。
- **上記の再認証は、判定の条件をむしろ良くした。** 現在のトークンは**すべて切り替え後に発行されたもの**なので、8.6 の「未確認の事項」（切り替え前に取得した refresh token が失効するか）はこの実機には当てはまらなくなった。**判定するのは「本番環境で発行したトークンが7日で失効しないこと」だけである。** → **2026-09-18 に達成を確認した（上記）。**
...
- ~~Google の7日ごと再認証が解けたことの実測~~ → **完了（2026-09-18、8.6）。** 8日16時間43分を経過して再認証を求められないことを確認した。**1.1.0 の作業とは独立である。**
