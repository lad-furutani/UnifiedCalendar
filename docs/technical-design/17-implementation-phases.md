...
- Google の7日ごと再認証が解けたことの実測（2026-09-15 以降、8.6）。**1.1.0 の作業とは独立である。**
- **次版の候補。** ステータス領域の3つのボタン（警告・手動更新・設定）の高さを揃える。現在は手動更新だけが自動サイズである。**1.0.0からの見た目を変える判断と、フォント10 / 24 DIPでの文字の収まりの確認が必要である。**

### 出荷の状態（2026-09-09 時点）

**文書とバージョンは 1.1.0 として整っているが、配布物はまだ出来ていない。**

| 項目 | 状態 |
| --- | --- |
| 公開文書・`csproj` | **完了**（`6c32557`） |
| `artifacts/publish/win-x64` と `UnifiedCalendar-Setup-1.1.0.exe` | **資格情報なし・未署名。配布不可。** `build.ps1`を資格情報なしで実行したため、`win-x64-client-credentials.json`は3件すべて`false`である。**配布前に資格情報を設定して`build.ps1`→`build-installer.ps1`（引数なし＝署名する）を通し直す。** この状態のインストーラを入れると、ビルドもインストールも成功するのに誰もアカウントを追加できない（8.7 / `acceptance-phase8.md` 0.2 と同じ形） |
| 出荷済み `UnifiedCalendar-Setup-1.0.0.exe` | **無傷**（2026-09-08 17:56） |
...
- **自動起動の登録先は、最後に起動したバイナリのパスになる。** 上記の事故の後もインストール先（1.0.0）を指しているため、**署名付き1.1.0をインストールするまで、ログオンのたびに同じ隔離が起きうる。**
